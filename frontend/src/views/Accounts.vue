<template>
  <div class="space-y-8">
    <section class="rounded-3xl border border-border bg-card p-6">
      <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div
          class="flex w-full flex-wrap items-center gap-3 text-xs text-muted-foreground sm:w-auto sm:flex-nowrap"
        >
          <Checkbox
          ...
          <span
            >已选 {{ selectedCount }} /
            {{ filteredAccounts.length }} 个账号</span
          >
      ...
      <div class="mt-4 flex flex-wrap items-center gap-2">
        <button
          class="rounded-full border border-border px-4 py-2 text-sm font-medium text-foreground transition-colors hover:border-primary hover:text-primary disabled:cursor-not-allowed disabled:opacity-50"
          :disabled="isLoading"
        ...
        <button
          class="rounded-full border border-border px-4 py-2 text-sm font-medium text-foreground transition-colors hover:border-primary hover:text-primary"
        ...
        >
          <button
            class="flex items-center gap-2 rounded-full border border-input bg-background px-4 py-2 text-sm font-medium text-foreground transition-colors hover:border-primary"
          ...
          >
            <button
              type="button"
              class="flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left text-sm text-foreground transition-colors hover:bg-accent"
              @click="
                openTaskModal();
                closeMoreActions();
              "
      ...
      <div
        v-if="viewMode === 'card'"
        class="mt-6 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3"
      >
        <div
        ...
        >
          ...
          <div
            class="mt-4 grid grid-cols-2 gap-3 text-xs text-muted-foreground"
          >
            <div>
              <p>剩余时间</p>
              <p
                class="mt-1 text-sm font-semibold"
                :class="remainingClass(account)"
              >
            ...
            <div>
              ...
              <p
                class="mt-1"
                :class="cooldownClass(account)"
              >
                <span v-if="account.cooldown_seconds > 0">
                  {{ formatCooldown(account.cooldown_seconds) }} ·
          ...
          <div class="mt-4 flex flex-wrap items-center gap-2">
            <button
              class="rounded-full border border-border px-3 py-1 text-xs text-foreground transition-colors hover:border-primary hover:text-primary"
            ...
            <button
              class="rounded-full border border-border px-3 py-1 text-xs text-destructive transition-colors hover:border-destructive hover:text-destructive"
              @click.stop
        ...
        <div
          ...
          class="rounded-2xl border border-border bg-background p-4 text-center text-xs text-muted-foreground"
        >
          暂无账号数据，请检查后台配置。
        </div>
      </div>
      ...
      <div
        v-else
        class="relative mt-6 overflow-x-auto overflow-y-visible"
      >
          ...
          <tbody class="text-sm text-foreground">
            ...
            >
              ...
              <td class="py-4 pr-6 text-xs">
                <span
                  v-if="account.cooldown_seconds > 0"
                  :class="cooldownClass(account)"
                >
                  {{ formatCooldown(account.cooldown_seconds) }} ·
                  {{ account.cooldown_reason }}
  ...
  <Teleport to="body">
    ...
    >
      <div
        class="flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden rounded-3xl border border-border bg-card shadow-xl"
      >
        <div
        ...
        <div class="scrollbar-slim flex-1 overflow-y-auto px-6 py-4">
          <div class="space-y-4 text-sm">
            <div
              class="flex rounded-full border border-border bg-muted/30 p-1 text-xs"
            >
              <button
                type="button"
            ...
            >
              ...
              />
              <div
                class="rounded-2xl border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground"
              >
            ...
            >
              ...
              <div
                v-if="importError"
                class="rounded-2xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs text-rose-600"
              >
                {{ importError }}
              </div>
            </div>
            ...
            <div
              class="rounded-2xl border border-rose-200 bg-rose-50 px-3 py-2 text-[11px] leading-relaxed"
            >
              ...
              <p class="mt-1 text-muted-foreground">
                详细声明请查看项目
                <a
                  href="https://github.com/Dreamy-rain/gemini-business2api/blob/main/docs/DISCLAIMER.md"
                  target="_blank"
  ...
  <Teleport to="body">
    ...
    >
      <div
        class="flex h-[80vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-border bg-card shadow-xl"
      >
        <div
          class="flex items-center justify-between border-b border-border/60 px-6 py-4"
        >
          ...
          <div class="flex items-center gap-2">
            <button
              class="rounded-full border border-border px-3 py-1 text-xs text-muted-foreground transition-colors hover:border-primary hover:text-primary disabled:cursor-not-allowed disabled:opacity-50"
              :disabled="
                !registerLogs.length &&
                !loginLogs.length &&
                !registerTask &&
        ...
        <div class="flex min-h-0 flex-1 flex-col px-6 py-4">
          <div
            ...
            class="rounded-2xl bg-destructive/10 px-3 py-2 text-xs text-destructive"
          >
            {{ automationError }}
          </div>

          <div
            v-if="registerTask || loginTask"
            class="grid gap-3 text-xs text-muted-foreground"
          >
            <div
              v-if="registerTask"
              class="space-y-1"
            ...
            >
              ...
              <div class="flex flex-wrap gap-x-4 gap-y-1">
                ...
                <span
                  >进度：{{ loginTask.progress }}/{{
                    loginTask.account_ids.length
                  }}</span
                >
            ...
            >
              ...
              >
                ...
                <div class="space-y-1 text-[11px] text-muted-foreground">
                  <div
                    v-for="(log, index) in registerLogs"
                    :key="`reg-${index}`"
                    class="font-mono"
                  >
                    {{ formatLogLine(log) }}
  ...
  <Teleport to="body">
    ...
    >
      <div
        class="w-full max-w-3xl rounded-3xl border border-border bg-card p-6 shadow-xl"
      >
        <div class="flex items-center justify-between">
          <p class="text-sm font-medium text-foreground">账户配置（JSON）</p>
          <div class="flex items-center gap-2">
            <button
              class="rounded-full bg-foreground px-3 py-1 text-xs text-background transition-opacity hover:opacity-90"
...
const filteredAccounts = computed(() => {
  ...
  return accounts.value.filter((account) => {
    ...
    const matchesStatus =
      ...
      statusLabel(account) === statusFilter.value;
...
const allSelected = computed(
  () =>
    ...
    filteredAccounts.value.every((account) =>
      selectedIds.value.has(account.id),
    ),
);
...
const syncRegisterTask = (task: RegisterTask | null, persist = true) => {
  ...
  if (
    task.id &&
    clearedRegisterTaskId.value &&
    task.id !== clearedRegisterTaskId.value
  ) {
...
const syncLoginTask = (task: LoginTask | null, persist = true) => {
  ...
  if (
    task.id &&
    clearedLoginTaskId.value &&
    task.id !== clearedLoginTaskId.value
  ) {
...
// 上次日志长度，用于判断是哪个日志在更新
let lastRegisterLogsLength = 0;
let lastLoginLogsLength = 0;

const scrollTaskLogsToBottom = async (scrollToRegister = false) => {
  await nextTick();
  const container = taskLogsRef.value;
  if (!container) return;

  if (scrollToRegister) {
    // 如果是注册日志更新，滚动到注册日志区域（容器顶部或注册日志末尾）
    // 查找注册日志的容器末尾位置
    const registerLogsContainer = container.querySelector(
      '[data-logs="register"]',
    );
    if (registerLogsContainer) {
      const scrollTarget =
        registerLogsContainer.getBoundingClientRect().bottom -
        container.getBoundingClientRect().top +
        container.scrollTop;
      container.scrollTop = Math.min(scrollTarget, container.scrollHeight);
      return;
    }
  }
...
watch(
  [registerLogs, loginLogs, isTaskOpen],
  async () => {
    if (!isTaskOpen.value) return;

    const currentRegisterLength = registerLogs.value.length;
    const currentLoginLength = loginLogs.value.length;

    // 判断是哪个日志在更新
    const registerUpdated = currentRegisterLength > lastRegisterLogsLength;
    const loginUpdated = currentLoginLength > lastLoginLogsLength;

    // 更新记录的日志长度
    lastRegisterLogsLength = currentRegisterLength;
    lastLoginLogsLength = currentLoginLength;

    // 只有当刷新日志更新或首次打开时才滚动到底部
    // 如果只有注册日志更新，滚动到注册日志区域
    if (loginUpdated || (!registerUpdated && !loginUpdated)) {
      await scrollTaskLogsToBottom(false);
    } else if (registerUpdated && !loginUpdated) {
...
const taskIndicatorClass = computed(() => {
  ...
  if (registerTask.value) {
    ...
    taskSummaries.push({
      ...
      fail,
      total,
      status: registerTask.value.status,
    });
  }
  if (loginTask.value) {
...
const statusLabel = (account: AdminAccount) => {
  if (
    account.cooldown_reason?.includes("429") &&
...
const openConfigPanel = async () => {
  ...
  try {
    const response = await accountsApi.getConfig();
    configData.value = Array.isArray(response.accounts)
      ? response.accounts
      : [];
...
const loadCurrentTasks = async () => {
  try {
    ...
    if (
      registerCurrent &&
      "id" in registerCurrent &&
      !isClearedRegisterTaskId(registerCurrent.id)
    ) {
      syncRegisterTask(registerCurrent);
      if (
        registerCurrent.status === "running" ||
        registerCurrent.status === "pending"
      ) {
  ...
  try {
    ...
    if (
      loginCurrent &&
      "id" in loginCurrent &&
      !isClearedLoginTaskId(loginCurrent.id)
    ) {
      syncLoginTask(loginCurrent);
      if (
        loginCurrent.status === "running" ||
        loginCurrent.status === "pending"
      ) {
...
const handleRegister = async () => {
  ...
  try {
    const count =
      Number.isFinite(registerCount.value) && registerCount.value > 0
        ? registerCount.value
        : undefined;
